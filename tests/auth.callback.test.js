...
// Plant a codeVerifier in the session before callback tests
...
app.get('/__test_get_session', (req, res) => {
  res.json({ user: req.session.user, codeVerifier: req.session.codeVerifier })
})
...
test('GET /auth/callback stores user in session and redirects on success', async () => {
...
test('GET /auth/callback clears codeVerifier after successful exchange', async () => {
  mockAuthenticateWithCodeAndVerifier.mockResolvedValueOnce({
    user: { firstName: 'Jane', lastName: 'Doe', email: 'jane@example.com' },
  })
